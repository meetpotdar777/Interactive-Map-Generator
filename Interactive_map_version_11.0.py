...
print("Creating a base map centered near London (global weather and AI insights enabled)...")
m = folium.Map(
...
random_points_group = folium.FeatureGroup(name='Random Points (AI Powered)').add_to(m) # Re-added random points
...
interesting_locations_group = folium.FeatureGroup(name='Interesting Locations (AI Powered)').add_to(m) # New FeatureGroup
...
# --- 13. Add Interesting Locations (AI-Powered) ---
# These are predefined locations that will fetch AI insights
print("Adding interesting locations with AI-powered insights...")
interesting_locations = [
    ...
    {"name": "Taj Mahal", "coords": [27.1751, 78.0421]}, # Already added, but for consistency if using this new logic
    {"name": "Red Fort, Delhi", "coords": [28.6562, 77.2410]}, # Already added, but for consistency if using this new logic
    {"name": "Sydney Opera House", "coords": [-33.8568, 151.2153]}
]

# The JavaScript will handle creating these markers and fetching AI insight on popup open
# We just need to make sure the JS part can access these. This is handled by creating markers
# in Python, and their popups will trigger the JS function when opened.
...
js_code = f"""
// ** IMPORTANT: REPLACE THESE PLACEHOLDER API KEYS WITH YOUR ACTUAL ONES **
// ** WITHOUT VALID KEYS, WEATHER AND AI FEATURES WILL NOT WORK CORRECTLY **

// OpenWeatherMap API Key (for weather data)
// Get yours here: https://openweathermap.org/api
const OPENWEATHERMAP_API_KEY = 'YOUR_OPENWEATHERMAP_API_KEY'; // <<< SET YOUR OPENWEATHERMAP API KEY HERE

// Google Gemini API Key (for AI insights and route analysis)
// Get yours here: https://aistudio.google.com/app/apikey
const GEMINI_API_KEY = 'YOUR_GEMINI_API_KEY'; // <<< SET YOUR GOOGLE GEMINI API KEY HERE
...
// Add basic CSS for toast messages dynamically
const toastCss = `
...
function getWeatherData(lat, lon, weatherDisplayElement) {{
    if (!OPENWEATHERMAP_API_KEY || OPENWEATHERMAP_API_KEY === 'YOUR_OPENWEATHERMAP_API_KEY') {{
        weatherDisplayElement.innerHTML = '<div style="color: red; font-weight: bold;">Error: OpenWeatherMap API Key is missing or invalid.</div>';
        showToast('Please set your OpenWeatherMap API Key for weather data!', 'error');
        return;
    }}
    ...
    weatherDisplayElement.innerHTML = 'Fetching weather...<br><div class="spinner-border spinner-border-sm text-info" role="status"><span class="visually-hidden">Loading...</span></span></div>';
...
// Function to get AI insight for a given location (Updated prompt for Wikipedia-like content)
async function getAIInsight(lat, lon, address, insightDisplayElement) {{
    if (!GEMINI_API_KEY || GEMINI_API_KEY === 'YOUR_GEMINI_API_KEY') {{
        insightDisplayElement.innerHTML = '<div style="color: red; font-weight: bold;">Error: Gemini API Key is missing or invalid.</div>';
        showToast('Please set your Google Gemini API Key for AI insights!', 'error');
        return;
    }}
    insightDisplayElement.innerHTML = 'Generating insight...<br><div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">Loading...</span></div>';

    const prompt = `Act as a concise Wikipedia entry. For the location at Latitude ${{lat.toFixed(4)}}, Longitude ${{lon.toFixed(4)}} (Address: "{{address}}"), identify the most relevant known place (city, region, landmark, etc.) and then provide a 1-2 sentence summary of what this specific place is famous for (historical, cultural, or geographical significance). Start directly with the place name.`;
    ...
    try {{
        ...
        if (result.candidates && result.candidates.length > 0 &&
            ...
            insightDisplayElement.innerHTML = `<p style="font-size: 0.85em; color: #333; margin-bottom: 0;">{{text}}</p>`;
        }} else {{
            insightDisplayElement.innerHTML = '<div style="color: #888; font-size: 0.85em; margin-bottom: 0;">No specific insight generated.</div>';
        }}
    }} catch (error) {{
        console.error("Error fetching AI insight:", error);
        insightDisplayElement.innerHTML = `<div style="color: red; font-size: 0.85em; margin-bottom: 0;">Error generating insight: ${{error.message}}.</div>`;
    }}
}}

// Function to analyze route with AI
async function analyzeRouteWithAI(startLat, startLon, endLat, endLon, routeAnalysisElement) {{
    if (!GEMINI_API_KEY || GEMINI_API_KEY === 'YOUR_GEMINI_API_KEY') {{
        routeAnalysisElement.innerHTML = '<div style="color: red; font-weight: bold;">Error: Gemini API Key is missing or invalid.</div>';
        showToast('Please set your Google Gemini API Key for AI route analysis!', 'error');
        return;
    }}
    routeAnalysisElement.innerHTML = 'Analyzing route...<br><div class="spinner-border spinner-border-sm text-success" role="status"><span class="visually-hidden">Loading...</span></div>';

    const prompt = `Provide a brief (2-3 sentences) conceptual analysis or interesting description of a hypothetical journey between Latitude ${{startLat.toFixed(4)}}, Longitude ${{startLon.toFixed(4)}} and Latitude ${{endLat.toFixed(4)}}, Longitude ${{endLon.toFixed(4)}}. Focus on potential scenery, challenges, or unique aspects, without providing directions.`;
    ...
    try {{
        ...
        if (result.candidates && result.candidates.length > 0 &&
            ...
            result.candidates[0].content.parts.length > 0) {{
            const text = result.candidates[0].content.parts[0].text;
            routeAnalysisElement.innerHTML = `<p style="font-size: 0.85em; color: #333; margin-bottom: 0;">{{text}}</p>`;
        }} else {{
            routeAnalysisElement.innerHTML = '<div style="color: #888; font-size: 0.85em; margin-bottom: 0;">No specific route analysis generated.</div>';
        }}
    }} catch (error) {{
        console.error("Error fetching AI route analysis:", error);
        routeAnalysisElement.innerHTML = `<div style="color: red; font-size: 0.85em; margin-bottom: 0;">Error analyzing route: ${{error.message}}.</div>`;
    }}
}}

// Function to create a marker that fetches AI insight on popup open
function createInsightMarker(lat, lng, name, group) {{
    var marker = L.marker([lat, lng], {{title: name}}).addTo(group);
    marker.bindPopup(`<b>${{name}}</b><br><div id="insight-popup-${{lat}}-${{lng}}" style="min-width: 150px;">Loading insight...<br><div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>`);
    marker.bindTooltip(name);

    marker.on('popupopen', function() {{
        const insightDivId = `insight-popup-${{lat}}-${{lng}}`;
        const insightDisplayElement = document.getElementById(insightDivId);
        if (insightDisplayElement) {{
            getAddressFromLatLng(lat, lng, function(address) {{
                getAIInsight(lat, lng, address, insightDisplayElement);
            }});
        }}
    }});
}}
...
m.on('click', function(e) {{
    ...
    getAddressFromLatLng(lat, lng, function(address) {{
        ...
        // Fetch and display AI insight for the clicked location
        const aiInsightDiv = document.getElementById('ai-insight');
        if (aiInsightDiv) {{
            getAIInsight(lat, lng, address, aiInsightDiv);
        }}
    }});
}});
...
// --- Custom Map Insights Control (Top Right) ---
var mapInsightsControl = L.control({{position: 'topright'}});
mapInsightsControl.onAdd = function (map) {{
    var div = L.DomUtil.create('div', 'info legend leaflet-control', 'map-insights-panel');
    div.style.backgroundColor = 'white';
    div.style.padding = '10px';
    div.style.borderRadius = '5px';
    div.style.boxShadow = '0 1px 5px rgba(0,0,0,0.4)';
    div.style.width = '280px'; // Slightly wider for more content
    ...
    div.innerHTML = `
        <h4 style="margin-top: 0; font-size: 1.2em; color: #333; text-align: center; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-bottom: 10px;"><i class="fa fa-brain"></i> Map Insights</h4>

        <div style="margin-bottom: 15px;">
            <p style="font-size: 0.8em; margin-bottom: 5px; color: #666;">Click on map for local weather & AI insight, or:</p>
            <button id="get-center-weather-btn" style="background-color: #28a745; color: white; border: none; padding: 8px 12px; border-radius: 4px; cursor: pointer; width: 100%; margin-bottom: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                <i class="fa fa-location-arrow"></i> Get Insights for Map Center
            </button>
            <div style="font-size: 0.9em; min-height: 50px; border: 1px solid #ddd; padding: 8px; border-radius: 3px; background-color: #fcfcfc;">
                <b style="color: #0056b3;">Weather:</b> <span id="weather-result">Click a location or button.</span>
                <b style="color: #6f42c1; display: block; margin-top: 8px;">AI Insight:</b> <span id="ai-insight">Click a location or button.</span>
            </div>
        </div>

        <div style="margin-top: 15px; border-top: 1px solid #eee; padding-top: 10px;">
            <h5 style="margin-top: 0; font-size: 1em; color: #333;"><i class="fa fa-route"></i> AI Route Analysis</h5>
            <p style="font-size: 0.8em; margin-bottom: 5px; color: #666;">Set start & end points on map:</p>
            <div class="d-flex justify-content-between gap-2 mb-2">
                <button id="set-start-point-btn" style="background-color: #007bff; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; flex: 1; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                    Set Start
                </button>
                <button id="set-end-point-btn" style="background-color: #dc3545; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; flex: 1; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                    Set End
                </button>
            </div>
            <div id="route-points-display" style="font-size: 0.8em; color: #555; margin-bottom: 10px;">
                Start: Not set<br>End: Not set
            </div>
            <button id="analyze-route-btn" style="background-color: #6f42c1; color: white; border: none; padding: 8px 12px; border-radius: 4px; cursor: pointer; width: 100%; box-shadow: 0 2px 5px rgba(0,0,0,0.1);" disabled>
                <i class="fa fa-robot"></i> Analyze Route with AI
            </button>
            <div id="route-analysis-result" style="font-size: 0.9em; min-height: 50px; border: 1px solid #ddd; padding: 8px; border-radius: 3px; background-color: #fcfcfc; margin-top: 10px;">
                AI analysis will appear here.
            </div>
        </div>

        <p style="font-size: 0.7em; color: #999; margin-top: 15px; text-align: center;">
            Powered by <a href="https://openweathermap.org/" target="_blank">OpenWeatherMap</a> & <a href="https://ai.google.dev/" target="_blank">Google Gemini AI</a>. <br>
            <span style="color: red; font-weight: bold;">Remember to add your API keys in the code!</span>
        </p>
    `;
...
mapInsightsControl.addTo(m);

# Append a div for toast messages to the body
var toastContainer = L.DomUtil.create('div', 'toast-container-custom');
toastContainer.id = 'toast-container';
document.body.appendChild(toastContainer);


# Add event listeners for the new buttons AFTER the control is added to the map
m.on('load', function() {{
    const getCenterWeatherBtn = document.getElementById('get-center-weather-btn');
    const weatherResultDiv = document.getElementById('weather-result');
    const aiInsightDiv = document.getElementById('ai-insight');
    const routePointsDisplay = document.getElementById('route-points-display');
    const analyzeRouteBtn = document.getElementById('analyze-route-btn');
    const routeAnalysisResult = document.getElementById('route-analysis-result');

    if (getCenterWeatherBtn) {{
        getCenterWeatherBtn.onclick = function() {{
            const center = m.getCenter();
            if (weatherResultDiv) {{
                getWeatherData(center.lat, center.lng, weatherResultDiv);
            }}
            if (aiInsightDiv) {{
                getAddressFromLatLng(center.lat, center.lng, function(address) {{
                    getAIInsight(center.lat, center.lng, address, aiInsightDiv);
                }});
            }}
        }};
    }}

    document.getElementById('set-start-point-btn').onclick = function() {{
        startPoint = m.getCenter();
        routePointsDisplay.innerHTML = `Start: ${{startPoint.lat.toFixed(4)}}, ${{startPoint.lng.toFixed(4)}}<br>End: ${{endPoint ? endPoint.lat.toFixed(4) + ', ' + endPoint.lng.toFixed(4) : 'Not set'}}`;
        if (startPoint && endPoint) analyzeRouteBtn.disabled = false;
        showToast('Start point set to map center.', 'info');
    }};

    document.getElementById('set-end-point-btn').onclick = function() {{
        endPoint = m.getCenter();
        routePointsDisplay.innerHTML = `Start: ${{startPoint ? startPoint.lat.toFixed(4) + ', ' + startPoint.lng.toFixed(4) : 'Not set'}}<br>End: ${{endPoint.lat.toFixed(4)}}, ${{endPoint.lng.toFixed(4)}}`;
        if (startPoint && endPoint) analyzeRouteBtn.disabled = false;
        showToast('End point set to map center.', 'info');
    }};

    analyzeRouteBtn.onclick = function() {{
        if (startPoint && endPoint) {{
            analyzeRouteWithAI(startPoint.lat, startPoint.lng, endPoint.lat, endPoint.lng, routeAnalysisResult);
        }} else {{
            showToast('Please set both start and end points first.', 'warning');
        }}
    }};
}});
...
# --- Add Interesting Locations (AI-Powered) to the map ---
print("Adding markers for interesting locations to the map...")
interesting_locations = [
    {"name": "Eiffel Tower", "coords": [48.8584, 2.2945]},
    {"name": "Statue of Liberty", "coords": [40.6892, -74.0445]},
    {"name": "Colosseum", "coords": [41.8902, 12.4922]},
...
for loc_data in interesting_locations:
    # Use a generic icon for these AI-powered points
    folium.Marker(
        location=loc_data["coords"],
        tooltip=loc_data["name"],
        icon=folium.Icon(color='purple', icon='info', prefix='fa')
    ).add_to(interesting_locations_group)
    # The JavaScript `createInsightMarker` function will be called on `popupopen`
    # for these markers to fetch and display AI insights.
    # We pass the coordinates and name to the createInsightMarker JS function.
    js_code_for_marker_creation = f"createInsightMarker({loc_data['coords'][0]}, {loc_data['coords'][1]}, '{loc_data['name']}', interesting_locations_group);"
    folium.Html(js_code_for_marker_creation).add_to(m)


# --- Add Random Points (AI-Powered) to the map ---
print("Adding random points with AI-powered insights to the map...")
for i in range(50): # Generate 50 random points
    lat = random.uniform(-60.0, 80.0) # Latitude range for global
    lon = random.uniform(-180.0, 180.0) # Longitude range for global
    point_name = f"Random Point {i+1}" # A generic name for the random point

    folium.Marker(
        location=[lat, lon],
        tooltip=f"Click for details on {point_name}",
        icon=folium.Icon(color='darkgreen', icon='question', prefix='fa') # Use a different icon for random points
    ).add_to(random_points_group)
    # The `createInsightMarker` function will automatically handle fetching AI insight on popup open.
    # The 'address' will be reverse geocoded by the JS function.
    js_code_for_random_marker_creation = f"createInsightMarker({lat}, {lon}, '{point_name}', random_points_group);"
    folium.Html(js_code_for_random_marker_creation).add_to(m)
...
# The Python variable `openweathermap_api_key` below is just a placeholder and not directly used
# by the JavaScript for tile layers; the JS const `OPENWEATHERMAP_API_KEY` is used.
openweathermap_api_key = 'YOUR_OPENWEATHERMAP_API_KEY' # This is just a placeholder here, the JS one is used.
