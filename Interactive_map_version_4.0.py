...
from folium.plugins import (
    ...
    HeatMap, MarkerCluster, TimestampedGeoJson
)
...
# The Choropleth layer will be added directly to the map but still manageable via LayerControl.
heatmap_group = folium.FeatureGroup(name='Simulated Heatmap').add_to(m) # New group for heatmap
marker_cluster_group = folium.FeatureGroup(name='Clustered Locations').add_to(m) # New group for marker cluster
timestamp_geojson_group = folium.FeatureGroup(name='Temporal Data (Timestamps)').add_to(m) # New group for timestamped geojson
image_overlay_group = folium.FeatureGroup(name='Historical Map Overlay').add_to(m) # New group for image overlay
...
# And also add it to its FeatureGroup for layer control, if needed (it will be linked by name)
# Note: Adding directly to 'm' and then to 'timestamp_geojson_group' might lead to duplicates
# in LayerControl if not careful. The primary fix is adding to 'm'.
# For cleaner LayerControl, you might just add to 'm' and rely on LayerControl's auto-discovery of named layers.
...
# --- 13. Add Custom JavaScript Interaction ---
# This adds a simple click listener to the map that logs coordinates to the browser console.
# You can use this to debug or trigger custom frontend actions.
print("Adding custom JavaScript interaction...")
js_code = """
...
m.on('click', function(e) {
    ...
    // Example of adding a temporary popup on click (not part of saved state)
    L.popup()
        .setLatLng(e.latlng)
        .setContent("You clicked here!<br>Lat: " + lat.toFixed(4) + "<br>Lng: " + lng.toFixed(4))
        .openOn(m);
});

// You can also add more advanced JS like changing layer properties dynamically:
// For example, changing the opacity of 'London Landmarks' group when button is clicked (hypothetical button)
// document.getElementById('some_button_id').onclick = function() {
//    var landmarksLayer = m._layers[Object.keys(m._layers).find(key => m._layers[key].options && m._layers[key].options.name === 'London Landmarks')];
//    if (landmarksLayer) {
//        landmarksLayer.setOpacity(landmarksLayer.options.opacity === 1 ? 0.5 : 1);
//        console.log("London Landmarks opacity toggled!");
//    }
// };
"""
...
# --- 14. Add core plugins for enhanced interactivity (re-ordered for logical flow) ---
...
# --- 15. Add the Layer Control to the map ---
