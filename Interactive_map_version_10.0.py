...
const OPENWEATHERMAP_API_KEY = ''; // <<< PUT YOUR OPENWEATHERMAP API KEY HERE

// ** IMPORTANT: Replace 'YOUR_GEMINI_API_KEY' with your actual Google Gemini API key **
// You can get a key from Google AI Studio: https://aistudio.google.com/app/apikey
const GEMINI_API_KEY = ''; // <<< PUT YOUR GOOGLE GEMINI API KEY HERE
...
function getWeatherData(lat, lon, weatherDisplayElement) {{
    if (!OPENWEATHERMAP_API_KEY) {{
        weatherDisplayElement.innerHTML = '<div style="color: red; font-weight: bold;">Error: Please provide your OpenWeatherMap API Key.</div>';
        return;
    }}
...
// Function to get AI insight for a given location
async function getAIInsight(lat, lon, address, insightDisplayElement) {{
    if (!GEMINI_API_KEY) {{
        insightDisplayElement.innerHTML = '<div style="color: red; font-weight: bold;">Error: Gemini API Key not set.</div>';
        return;
    }}
    ...
    const prompt = `Provide a very brief (1-2 sentences) interesting fact, historical note, or cultural significance about the location at Latitude ${{lat.toFixed(4)}}, Longitude ${{lon.toFixed(4)}} (Address: ${{address}}). If it's a generic area, provide a general geographic or climate insight.`;
...
async function analyzeRouteWithAI(startLat, startLon, endLat, endLon, routeAnalysisElement) {{
    if (!GEMINI_API_KEY) {{
        routeAnalysisElement.innerHTML = '<div style="color: red; font-weight: bold;">Error: Gemini API Key not set.</div>';
        return;
    }}
...
m.on('load', function() {{
    ...
    document.getElementById('set-start-point-btn').onclick = function() {{
        ...
        alert('Start point set to map center. Now set end point or click to analyze.');
    }};
    ...
    document.getElementById('set-end-point-btn').onclick = function() {{
        ...
        if (startPoint && endPoint) analyzeRouteBtn.disabled = false;
        alert('End point set to map center. Click "Analyze Route with AI".');
    }};
